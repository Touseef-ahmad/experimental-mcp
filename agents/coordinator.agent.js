 ...
 */
export function createCoordinatorAgent(employeeAgent, analyticsAgent, reportingAgent, approvalAgent) {
    ...
    const employeeHandoff = handoff(employeeAgent.agent, {
        ...
        toolDescriptionOverride: "Transfer to employee specialist for employee data, team info, and org structure queries.",
    });
    const analyticsHandoff = handoff(analyticsAgent.agent, {
        toolNameOverride: "transfer_to_analytics",
        toolDescriptionOverride: "Transfer to analytics specialist for metrics, trends, engagement scores, and project health.",
    });
    const reportingHandoff = handoff(reportingAgent.agent, {
        toolNameOverride: "transfer_to_reporting",
        toolDescriptionOverride: "Transfer to reporting specialist for generating reports and summaries.",
    });
    const approvalHandoff = handoff(approvalAgent.agent, {
        toolNameOverride: "transfer_to_approval",
        toolDescriptionOverride: "Transfer to approval specialist for approval workflows and risk assessments.",
    });
