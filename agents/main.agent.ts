...
export function createMainAgent(config: MainAgentConfig) {
  ...
  const employeeHandoff = handoff(employeeAgent.agent, {
    ...
    toolDescriptionOverride:
      "Delegate to employee specialist for employee data, team info, and org structure queries.",
  });

  const analyticsHandoff = handoff(analyticsAgent.agent, {
    toolNameOverride: "delegate_to_analytics",
    toolDescriptionOverride:
      "Delegate to analytics specialist for metrics, trends, engagement scores, and project health.",
  });

  const reportingHandoff = handoff(reportingAgent.agent, {
    toolNameOverride: "delegate_to_reporting",
    toolDescriptionOverride:
      "Delegate to reporting specialist for generating reports and summaries.",
  });

  const approvalHandoff = handoff(approvalAgent.agent, {
    toolNameOverride: "delegate_to_approval",
    toolDescriptionOverride:
      "Delegate to approval specialist for approval workflows and risk assessments.",
  });

  const coordinatorHandoff = handoff(coordinatorAgent.agent, {
    toolNameOverride: "delegate_to_coordinator",
    toolDescriptionOverride:
      "Delegate to coordinator for complex multi-agent orchestration tasks.",
  });
